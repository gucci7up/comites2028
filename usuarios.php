...
$subtitulo_pagina = 'Administración de accesos al sistema';
include 'includes/header.php';
?>

<style>
.role-badge { display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px; }
.role-admin      { background:var(--accent-tint);color:var(--accent); }
.role-supervisor { background:var(--warning-bg);color:var(--warning); }
.role-user       { background:var(--bg);color:var(--text-secondary); }
.estado-dot { display:inline-flex;align-items:center;gap:6px;font-size:12.5px;color:var(--success); }
.estado-dot span { width:7px;height:7px;border-radius:50%;background:var(--success); }
.user-avatar-sm { width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent-light));color:#fff;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0; }
</style>
            ...
            <tr>
                <td>
                    <div class="d-flex align-items-center gap-2">
                        <div class="user-avatar-sm"><?php echo strtoupper(substr($u['nombre'],0,1)); ?></div>
                        <div>
                            <div style="font-weight:600;font-size:13.5px;"><?php echo htmlspecialchars($u['nombre']); ?></div>
                            <div style="font-size:11px;color:var(--text-tertiary);">@<?php echo htmlspecialchars($u['usuario']); ?></div>
                        </div>
                    </div>
                </td>
                ...
                <td><span class="estado-dot"><span></span>Activo</span></td>
                <td style="font-size:12px;color:var(--text-tertiary);"><?php echo date('d/m/Y', strtotime($u['fecha_registro'])); ?></td>
                <td>
            ...
            <div class="modal-body">
                <form id="usuarioForm" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="fld" id="nombre" name="nombre" required>
                    </div>
                    <div class="mb-3">
                        <label class="lbl">Email</label>
                        <input type="email" class="fld" id="email" name="email" required>
                    </div>
                    <div class="mb-3">
                        <label class="lbl">Nombre de Usuario</label>
                        <input type="text" class="fld" id="usuario" name="usuario" required>
                    </div>
                    <div class="mb-3">
                        <label class="lbl">Contraseña</label>
                        <input type="password" class="fld" id="password" name="password">
                        <div class="form-text mt-1">Dejar en blanco para mantener la contraseña actual.</div>
                    </div>
                    <div class="mb-4">
                        <label class="lbl">Rol</label>
                        <select class="fld" id="rol" name="rol" required>
                            <option value="digitador">Digitador</option>
                            <option value="supervisor">Supervisor</option>
                            <option value="admin">Administrador</option>
                        </select>
                    </div>
                    <button type="submit" name="guardar_usuario" class="btn btn-primary w-100">
